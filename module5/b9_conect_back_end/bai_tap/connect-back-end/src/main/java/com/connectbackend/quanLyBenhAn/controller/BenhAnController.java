package com.connectbackend.quanLyBenhAn.controller;

import com.connectbackend.quanLyBenhAn.entity.BenhAn;
import com.connectbackend.quanLyBenhAn.entity.BenhNhan;
import com.connectbackend.quanLyBenhAn.repository.BenhNhanRepository;
...
//    }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<BenhAn> delete(@PathVariable("id") Integer id) {
        Optional<BenhAn> benhAn = iBenhAnService.findById(id);
        if (benhAn == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
